...
async function anilist(user) {
          ...
          <div class="flex  items-end gap-16  w-full">
           <div class="w-full">
            ...
            <div class="w-full h-4 bg-[#272727] rounded-sm py-[2px] px-[4px]">
             ...
             <span class="text-xs">${dateFormat(new Date(item.updatedAt * 1000).toISOString())}</span>
             </div>
           </div>
          </div>
        </div>`;
    ...
    <h3 class="text-sm font-light">Last Anime Updates</h3>
    <div class="mt-2 w-full flex flex-col gap-2">
      ${user.animeList.nodes.length === 0 ? "<span class='text-[#9ca3af] text-sm'>No recent updates found.</span>" : ""}
